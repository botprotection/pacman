...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for var in [OBS0, OBS1]:
        variableDomainsDict[var] = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factors, newFactor = joinFactorsByVariable(factors, var)

            if len(newFactor.unconditionedVariables()) >= 2:
                tempFactor = eliminate(newFactor, var)
                factors.append(tempFactor)

        factor = joinFactors(factors)
        factors = normalize(factor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for i, prob in enumerate(distribution):
            cumulative_total += prob
            if cumulative_total >= choice:
                return values[i]
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        noisyProb = busters.getObservationProbability(noisyDistance, trueDistance)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        distribute = DiscreteDistribution({pos: self.getObservationProb(observation, pacPosition, pos, jailPosition) * self.beliefs[pos] for pos in self.allPositions})
        distribute.normalize()

        self.beliefs = distribute
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDistribution.keys():
                new_beliefs[newPos] += oldProb * newPosDistribution[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particles_per_position = self.numParticles // len(self.legalPositions)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if count.total() == 0:
            self.initializeUniformly(gameState)
            count = self.getBeliefDistribution()

        for i in range(self.numParticles):
            newSample = count.sample()
            self.particles[i] = newSample
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        posPos = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(posPos)

        n = self.numParticles

        while n > 0:
            self.particles += posPos[:min(n, len(posPos))]
            n -= len(posPos)
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if ghostDist is None:
                    particle = list(particle)
                    particle[i] = jailPos
                    particle = tuple(particle)
                    prob = 0
                    prob *= self.getObservationProb(ghostDist, pacPos, particle[i], jailPos)
                else:
                    prob *= self.getObservationProb(ghostDist, pacPos, particle[i], jailPos)
        ...
        self.particles = [count.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
