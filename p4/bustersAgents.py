...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions:
            tempPos = dist.argMax()
            tempDist = self.distancer.getDistance(pacmanPosition, tempPos)
            if not distance or distance > tempDist:
                distance = tempDist
                ghostPos = tempPos
        ...
        for a in legal:
            temp2Pos = Actions.getSuccessor(pacmanPosition, a)
            temp2Dist = self.distancer.getDistance(temp2Pos, ghostPos)
            if not closest_ghost_distance or closest_ghost_distance > temp2Dist:
                closest_ghost_distance = temp2Dist
                best_action = a
